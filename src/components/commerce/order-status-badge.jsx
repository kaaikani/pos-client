...
import { ShoppingCart, CreditCard, Clock, CheckCircle, Truck, PackageCheck, Package, XCircle, } from 'lucide-react';
const STATUS_CONFIG = {
...
export function OrderStatusBadge({ state }) {
    const config = STATUS_CONFIG[state] || { color: 'bg-gray-100 text-gray-800', label: state, icon: Clock };
    const Icon = config.icon;
    return (<Badge className={config.color} variant="secondary">
