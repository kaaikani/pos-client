...
export function FacetFilters({ productDataPromise }) {
    const result = use(productDataPromise);
    const searchResult = result.data.search;
    const pathname = usePathname();
    const searchParams = useSearchParams();
    const router = useRouter();
    const facetGroups = searchResult.facetValues.reduce((acc, item) => {
    ...
    return (<div className="space-y-6">
            ...
            {Object.entries(facetGroups).map(([facetName, facet]) => (<div key={facet.id} className="space-y-3">
                    <h3 className="font-medium text-sm">{facetName}</h3>
                    <div className="space-y-2">
                        {facet.values.map((value) => {
                const isChecked = selectedFacets.includes(value.id);
                return (<div key={value.id} className="flex items-center space-x-2">
                                    <Checkbox id={value.id} checked={isChecked} onCheckedChange={() => toggleFacet(value.id)}/>
